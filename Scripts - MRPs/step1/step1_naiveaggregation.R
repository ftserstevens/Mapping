...
raw_1for1 = function(df, nseed = 0) {
  ...
  df_onlyvote$Candidate2020 = factor(df_onlyvote$Candidate2020)
  all_idstr = unique(df_onlyvote$id_str)
  all_count = rep(NA, length(all_idstr))
  df_1for1_raw = df_onlyvote[0,] #empty df init
  ...
  for (i in 1:length(all_idstr)) {
    id_str.i = unique(df_onlyvote$id_str)[i]
    df_idstr.i = df_onlyvote[df_onlyvote$id_str == id_str.i,]
    table.i = table(df_idstr.i$Candidate2020)
    min_1for1 = min(table.i)
    all_count[i] =  (min_1for1*2)
    
    if(0 %in% table.i) {next}## exclude id_str if no representation of 1 vote
    
    df_1for1_raw.b = df_onlyvote[0,] #empty df init
    for (b in 1:10) {#bootstrap 

      df_idstr.i.biden = dplyr::sample_n(
        df_idstr.i[df_idstr.i$Candidate2020 == "Joe Biden",], size = min_1for1)
      df_idstr.i.trump = dplyr::sample_n(
        df_idstr.i[df_idstr.i$Candidate2020 == "Donald Trump",], size = min_1for1)
...
#Partisan
dem = df$Candidate2020 == "Joe Biden"
rep = df$Candidate2020 == "Donald Trump"
